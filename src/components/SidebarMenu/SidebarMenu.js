...
function SidebarMenu(props) {
  return (
    <aside
      className={
        "w-[25vw] px-5 flex flex-col justify-between items-start max-w-[300px] h-screen"
      }
    >
      <div className={"flex flex-col justify-start items-center gap-3"}>
        <h2 className={"uppercase"}>Energy 2.0</h2>
      ...
      <nav className={"flex flex-col justify-between items-start gap-0.5"}>
        {Object.entries(menuStructure).map(([key, buttons], index) => (
          <React.Fragment>
            <h3 className={"text-[1.5rem] font-semibold"}>{key}</h3>
            {buttons.map((button) => (
              <Link to={button.link}>
      ...
      <StyledButton endIcon={<Settings />}>Administrator</StyledButton>
    </aside>
  );
}
