...
function AddConsumer(props) {
    ...
    useEffect(() => {
        if (consumerId) {
            apiRequest({method: "get", url: "consumers/" + consumerId + "/"}).then(res => {
                setValues(res.data)
    ...
    return (
        <div>
            <h2 className={"page-title"}>Kunde {consumerId ? "bearbeiten" : "hinzufügen"}</h2>
            <WidgetComponent>
                <form className={"flex flex-col gap-4 px-4"}>
                    <TextField name={"name"} value={values.name} onChange={handleChange} placeholder={"Name"}
                               label={"Name"}/>
                    <TextField name={"user.username"} value={values.user.username} onChange={handleNestedChange}
                               placeholder={"Benutzername"} label={"Benutzername"}/>
                    <TextField name={"user.password"} value={values.user.password} onChange={handleNestedChange}
                    ...
                    <TextField name={"phone"} value={values.phone} onChange={handleChange}
                               placeholder={"Telefonnummer"} label={"Telefonnummer"}/>
                    <TextField name={"producer"} value={values.producer} onChange={handleChange}
                               placeholder={"Produzent"} label={"Produzent"}/>
                    <TextField name={"sensor.device_id"} value={values.sensor.device_id} onChange={handleNestedChange}
                               placeholder={"Sensorzählernummer"} label={"Sensorzählernummer"}/>
                    <TextField name={"rates"} value={values.rates} onChange={handleChange}
                               placeholder={"Tarife"} label={"Tarife"}/>
                    <StyledButton variant={"contained"} color={"primary"} onClick={handleSave}>
                        {consumerId ? "Speichern" : "Anlegen"}
                    </StyledButton>
                    <Button variant={"contained"} color={"secondary"} onClick={() => navigate("/kunden")}>
                        Abbrechen
                    </Button>
                </form>
            </WidgetComponent>
